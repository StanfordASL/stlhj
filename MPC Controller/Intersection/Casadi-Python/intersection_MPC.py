...
class MPC:
    ...
    def loadOptimalStates(self):
        ...
        for i in range(np.size(t)):
            ...
            for j in range(self.integration_iters):
                ...
                temp_states[1] = temp_states[1] + (temp_states[3]*np.sin(temp_states[2]))*dt2
    ...
    def loadSingleOptimalStates(self,states):
        ...
        if self.interp_function_V1([states[5],states[4],states[3],states[2],states[1],states[0]]) > self.interp_function_V2([states[5],states[4],states[3],states[2],states[1],states[0]]):
            out_array_th = self.interp_function_V1(flat_for_th.T)
            out_array_vel = self.interp_function_V1(flat_for_vel.T)
        else:
            out_array_th = self.interp_function_V2(flat_for_th.T)
            out_array_vel = self.interp_function_V2(flat_for_vel.T)
