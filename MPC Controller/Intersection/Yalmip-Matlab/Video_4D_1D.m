...
%traj_temp = [0.2,-0.4,pi/2,0.,0.5]; %no pass
traj_temp = [-0.0,-0.4,pi/2,0.15,0.4]; %danger
%traj_temp = [0.2,-0.4,pi/2,0.15,0.4];
%traj_temp = [0.2,-0.4,pi/2,0.15,0.4];%pass
...
%                     deriv_R1{4}(:,:,:,:,:,end+2-I_end),deriv_L1{4}(:,:,:,:,:,end+2-I_end),tstep_sim,value1);
            [Vq_th_mat,Vq_v_mat] = eval_valuefunction_for_PID_Controller(traj_temp(end,:),alpha_U_beta1{end+2-I_end}(:,:,:,:,:),th_mat,v_mat,gmatOut1);
            [u(j,1:2),integral] = PID_Controller(traj_temp(end,3),traj_temp(end,4),Vq_th_mat,Vq_v_mat,tstep_sim,th_mat,v_mat,integral);
...
%                     deriv_R2{4}(:,:,:,:,:,end+2-I_end),deriv_L2{4}(:,:,:,:,:,end+2-I_end),tstep_sim,value2);
                      [Vq_th_mat,Vq_v_mat] = eval_valuefunction_for_PID_Controller(traj_temp(end,:),alpha_U_beta2{end+2-I_end}(:,:,:,:,:),th_mat,v_mat,gmatOut2);
            [u(j,1:2),integral] = PID_Controller(traj_temp(end,3),traj_temp(end,4),Vq_th_mat,Vq_v_mat,tstep_sim,th_mat,v_mat,integral);
