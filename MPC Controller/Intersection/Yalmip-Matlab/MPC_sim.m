...
states(1,:) = [0.2,-0.4,pi/2,0.,0.4,0]; %Pass
%states(1,:) = [0.2,-0.45,pi/2,0.,0.5,0]; %NoPass
%states(1,:) = [-0.01,-0.4,pi/2,0.15,0.4,0]; %Min. Violation
traj_temp = states(1,:);
v2 = -0.15;
%% Outputs
...
OptStates_Set = zeros(12/p.dt,2);
MaxValueFunction = ones(12/p.dt+1,1);
for i = 1:12/p.dt
    tic
    [OptStates, MaxValueFunction(i+1)] = loadOptimalStates(states(i,:),V1,V2,p,thd,vd,v2,integration,integration_iters);
    ...
    if states(i+1,5) <= -0.55
        states(i+1,5) = -0.55;
        traj_temp(end,5) = -0.55;
    end
    toc
end
