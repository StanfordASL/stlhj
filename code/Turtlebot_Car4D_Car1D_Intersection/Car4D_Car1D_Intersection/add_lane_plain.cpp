void add_lane_plain(
  ...
  beacls::FloatVec range){
    ...
    if (dim == 0){
      ...
      // lane_temp0.assign(xs.size(),0.);
      // lane_temp1.assign(xs.size(),0.);
      // theta_atan2.assign(xs.size(),0.);
      //
      // std::transform(xs0.cbegin(), xs0.cend(), lane_temp0.begin(),
      // [lane_width,range](const auto &xs_i) {
      //   return 2.*(1. - std::pow(((xs_i - (range[1]+range[0])/2.)/(lane_width/2.)),2)); });
      //
      //   std::transform(xs1.cbegin(), xs1.cend(), lane_temp1.begin(),
      //   [lane_width,range](const auto &xs_i) {
      //     return 2.*(1. - std::pow(((xs_i - (range[2]+range[3])/2.)/(lane_width/2.)),2)); });
      //
      //     std::transform(xs0.cbegin(), xs0.cend(), xs1.cbegin(), lane.begin(),
      //     [lane_width,range,r](const auto &xs0_i,const auto &xs1_i) {
      //       return 2.*(1.-std::pow(std::abs(std::sqrt(std::pow(xs0_i,2)+std::pow(xs1_i,2))-r)/(lane_width/2.),2)); });
      //
      ...
      //             return (std::atan2(xs1_i,xs0_i)+2*M_PI)/(2*M_PI)  ; });
      //
