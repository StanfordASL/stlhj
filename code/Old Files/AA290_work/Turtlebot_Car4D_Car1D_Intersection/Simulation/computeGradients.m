function [derivC, derivL, derivR] = computeGradients_C(g, data, dims, derivFunc)
...
for i = 1:g.dim
  if dims(i)
    %derivC{i} = zeros(size(data),'single');
    derivL{i} = zeros(size(data),'single');
    %derivR{i} = zeros(size(data),'single');
    %% data at a single time stamp
    if tau_length == 1
      % Compute gradient using level set toolbox
      [derivL{i}, derivR] = derivFunc(g, data, i);
      ...
      for t = 1:tau_length
        [derivL{i}, derivR] = derivFunc(g, data(colons{:}, t), i);
